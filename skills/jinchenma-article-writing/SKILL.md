...
description: 文章写作前的素材整理、大纲协作与金尘马式 AI 干货写作规则加载。用户说“我要写一篇文章”“帮我做这个选题”“先搜素材”“全网搜攻略/教程/资料并保存”“写文章前先整理资料”“调整大纲”“我们讨论一下文章结构”，或提到“AI 干货”“面向小白讲清楚”“写得实用能收藏”“不要太技术要讲人话”时，必须使用本 skill。这个 skill 负责确认选题、明确素材来源、整理素材卡、建立素材索引、把讨论收敛成结果态大纲，并在 AI 干货写作场景下读取公开文风参考；没有用户明确提供素材地址时，必须先询问素材地址，不要自行猜测或默认选择任何本地目录。
---
...
- 公开版只读取本 skill 内的公开参考文件；如果用户有自己的风格文档，由用户额外提供路径。
...
如果用户有自己的风格文档、平台规范或写作样本，可以让用户额外提供路径；只有用户明确提供时才读取。不要临时猜测私人目录，也不要默认读取本仓库外的任何本地资料。
...
干货长文里真正能被读者截图、转发、二次传播的句子，通常只有几句。直接发原文，读者扫读时容易抓不住重点。先把核心判断句、反差句、操作提醒和收尾 CTA 标出来，能显著提升扫读效率和传播感。
...
不要依赖外部私有 skill。用户确认后，直接基于当前定稿和 `references/writing-style.md` 做通用标记。

默认使用 Markdown 加粗标记，除非用户指定其他格式。

优先标这些句子：

- 核心判断句：表达文章主张、取舍标准或关键结论。
- 反差句：纠正常见误区，制造“原来不是这样”的理解转折。
- 操作提醒：真正影响结果的步骤、检查点、避坑标准。
- 收尾 CTA：引导读者保存、实践、复盘或继续行动的句子。

约束：

- 默认加粗不超过全文 10%。
- 每个自然小节最多标 1-2 处，避免整篇到处都是重点。
- 不加粗铺垫句、解释句、纯情绪句和重复表达。
- 不为了传播感改写事实，不制造夸张承诺。
